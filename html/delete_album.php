<?php
  ...
  if (( isset( $_GET['album'] ) ) && ( $_SESSION['admin_flag'] == 'true' )) {
     $msg = 'You are an admin and able to delete this album';
      if ( !( $stmt = $db->prepare( "SELECT `albums`.`album_id`, `albums`.`album_name` FROM `albums` JOIN `permissions` ON `albums`.`album_id`=`permissions`.`album_id` WHERE `albums`.`album_id` = ?;" ) ) ) {
        die( 'Prepare failed: (' . $db->errno . ') ' . $db->error );
      } else {
          # Bind the variables into the prepared statement
          $start = $page * $albums_per_page;
          if ( !$stmt->bind_param( 'i', $_GET['album'] ) ) {
